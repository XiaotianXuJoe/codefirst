...
const ENV_CONFIGS: Record<string, EnvConfig> = {
  ...
  'ai-chat': {
    ...
    pipCommand: 'pip install requests python-dotenv',
    installInstructions: [
      '确保已安装 Python 3.8+',
    ...
    runCommand: 'python chat.py',
    runInstructions: [
      '打开终端，进入项目文件夹',
      '复制 .env.example 为 .env 并填入 API Key',
      '运行: python chat.py',
      '输入问题与 AI 对话，输入 quit 退出',
    ],
  },
  'chat-history': {
    hasDependencies: true,
    ...
    runInstructions: [
      ...
      '运行: python chat.py',
      '输入 /help 查看所有命令',
      '支持 /new /list /switch /system /personality /clear',
    ],
  },
  'tool-agent': {
    hasDependencies: true,
    ...
    runInstructions: [
      ...
      '复制 .env.example 为 .env 并填入 API Key',
      '运行: python agent.py',
      '尝试提问: "北京天气怎么样"、"计算 15*23"、"现在几点了"',
    ],
  },
  'rag-kb': {
    hasDependencies: true,
    runtime: 'python',
    dependencies: ['requests', 'python-dotenv', 'chromadb'],
    pipCommand: 'pip install requests python-dotenv chromadb',
    installInstructions: [
      '确保已安装 Python 3.8+',
      '安装依赖: pip install requests python-dotenv chromadb',
      '复制 .env.example 为 .env',
      '填入 DEEPSEEK_API_KEY（对话用）: https://platform.deepseek.com/',
      '填入 SILICONFLOW_API_KEY（Embedding用）: https://cloud.siliconflow.cn/',
      '注意: DeepSeek 不提供 Embedding API，Embedding 使用 SiliconFlow 免费服务',
    ],
    runCommand: 'python rag.py',
    runInstructions: [
      '打开终端，进入项目文件夹',
      '复制 .env.example 为 .env 并填入两个 API Key',
      '运行: python rag.py',
      '使用 /upload 上传文档，或 /upload_dir 批量上传文件夹',
      '上传完成后直接输入问题即可进行 RAG 问答',
      '使用 /help 查看所有命令',
    ],
  },
}
